...
fit.cf<-cforest(Survived ~ Age + Sex + Pclass  + family_size, data = train_new,controls=cforest_unbiased(ntree=500, mtry=3))

rf<-randomForest(Survived ~ Pclass + Sex + SibSp + Parch + Fare + Embarked + 
                  Title + family_size, data = train_new,importance=TRUE,ntree=500)
p = predict(rf,test_new)
